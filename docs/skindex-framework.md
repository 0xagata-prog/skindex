...
- 官网是聚合数据源：发现、筛选、预览完整皮肤与轻量配色，并提供 Manifest API 与审核队列。
- GitHub 是 Skill 源码和版本的唯一发布源；官网安装入口调用 Codex 内置 `$skill-installer` 读取 GitHub 的 `skill/` 目录。
- 当前直接分发 `$skindex` Skill；插件等公开上架后再开放。
- Codex Dream Skin 是当前完整皮肤核心运行基座；SkinDex 在上层统一编目、能力标注、审核和编排。
- Skill 是对话入口：推荐官网皮肤、区分完整皮肤与轻量配色、生成、投稿、恢复，并逐步接入可信运行时。
...
- 轻量配色通过后可写入公开主题目录；Dream Skin 等完整皮肤通过初审后进入人工编目，绝不自动转换成 `codex-theme-v1`。
- 投稿记录引擎、能力列表、实机验证状态和可选来源 URL；审核缩略图只在最终上架后开放公共读取。
